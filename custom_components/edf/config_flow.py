...
from .const import CONF_JS_ERROR, CONF_CODE_VERIFIER, CONF_NONCE, CONF_STATE, CONF_ACCESS_TOKEN, CONF_REFRESH_TOKEN, CONF_TOKEN_EXPIRATION, CONF_INSEE_CODE, CONF_BUSINESS_PARTNER, CONF_PDL, DOMAIN, DEFAULT_NAME
        ...
        if user_input is not None:
            ...
            if len(code) != 1 or len(resp_state) != 1:
                ...
                if resp_state[0] != self.data[CONF_STATE]:
                    ...
                    access_token, refresh_token, expiration = await self._auth.get_token(
                        code=code[0], code_verifier=self.data[CONF_CODE_VERIFIER], nonce=self.data[CONF_NONCE]
                    )
                    if access_token is not None:
                        accord_co, bp_num, address = await self._auth.get_person_data(access_token)
                        valid, pdl = await self._auth.get_pdl(access_token, accord_co, bp_num)
